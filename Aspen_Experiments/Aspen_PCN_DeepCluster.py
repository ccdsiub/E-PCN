...
parser = argparse.ArgumentParser(description='Aspen PCN DeepCluster: DGCNN + DeepCluster on ASPEN Open Jets (unsupervised)')
...
parser.add_argument('--model_architecture', type=str, default=f'DGCNN-DeepCluster-{batch_size}', help='Model architecture name')
parser.add_argument('--n_clusters', type=int, default=5, help='Number of clusters (default: 10)')
...
# Dataset: dynamic HDF5 graph construction (no disk storage)

class AspenHDF5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Skip jets with too few particles (same approach as original PCN pipeline)
            if n_particles < 2:
            ...
            # Stack all 14 features: [11 original, pT, deta, dphi]
            features = np.column_stack([
                particles,  # indices 0-10
                pT,         # index 11
                deta,       # index 12
                dphi        # index 13
            ]).astype(np.float32)
...
def collatePseudoLabels(batch):
    """For DeepCluster training. Returns (batched_graph, pseudo_labels)."""
    valid = [item for item in batch if item is not None]
    if len(valid) == 0:
        return None, None
    graphs = [item['graph'] for item in valid]
...
class DGCNNEncoder(nn.Module):
    """Base DGCNN encoder that outputs a 64-dim graph embedding."""
...
def collect_embeddings(encoder, data_loader, device):
    """Pass all data through encoder, return embeddings as numpy array."""
    encoder.eval()
    all_embeddings = []
    with torch.no_grad():
        for batched_graph in tqdm(data_loader, desc="Collecting embeddings", leave=False):
            if batched_graph is None:
                continue
            batched_graph = batched_graph.to(device)
            embedding = encoder(batched_graph)
            all_embeddings.append(embedding.cpu().numpy())
            del batched_graph, embedding
...
dataset = AspenHDF5Dataset(args.h5_path, knn_k=knn_k)
...
# Vectorized: read entire PFCands, count non-zero rows per jet in one pass
...
base_filename = f"Aspen_PCN_DeepCluster_{modelArchitecture}"
...
wandb.init(
    ...
    config={
        ...
        "model_type": "Base DGCNN (dynamic graphs, no BatchNorm)",
...
trainLoaderNoShuffle = DataLoader(
    ...
    collate_fn=collateGraphsOnly, drop_last=False,
    num_workers=0,
)
...
print("DeepCluster training (base DGCNN, dynamic graphs, ASPEN Open Jets)")

# Create encoder (random initialization)
encoder = DGCNNEncoder(in_feats, hidden_feats, chebFilterSize).to(device)
...
for iteration in range(maxIterations):
    ...
    pseudo_loader = DataLoader(
        ...
        collate_fn=collatePseudoLabels, drop_last=True,
        num_workers=0,
    )
    ...
    for graph, batch_pseudo_labels in tqdm(pseudo_loader, desc=f"  Training iter {iteration+1}", leave=False):
        if graph is None:
            continue
        ...
        embeddings_batch = encoder(graph)
        ...
        del graph, batch_pseudo_labels, embeddings_batch, logits, loss
...
print("Aspen PCN DeepCluster done.")
